    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `NotesTable` (`Id` INTEGER PRIMARY KEY AUTOINCREMENT, `Title` TEXT, `IsComplete` INTEGER)');
...
class _$NotesDao extends NotesDao {
  ...
  Future<List<NoteModel>?> getAllNotes() async {
    return _queryAdapter.queryList(
        'select * from NotesTable order by IsComplete',
  ...
  @override
  Future<void> changeNoteCompleteStatus(
    int id,
    bool value,
  ) async {
    await _queryAdapter.queryNoReturn(
        'update NotesTable set IsComplete = ?2 where id = ?1',
  ...
  Future<void> deleteNote(int id) async {
    await _queryAdapter
        .queryNoReturn('delete from NotesTable where id = ?1', arguments: [id]);
  }

  @override
  Future<void> insertNote(NoteModel note) async {
    await _noteModelInsertionAdapter.insert(note, OnConflictStrategy.replace);
  }
}
